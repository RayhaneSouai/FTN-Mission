...
addEventListener('message', ({ data }) => {
  try {
    const rows = data.rows as MappedUserRow[];
    const existingEmails = new Set<string>(data.existingEmails ?? []);
    const emailInFile = new Map<string, number>();
    const results: RowValidationResult[] = [];
    const payloads: Array<AdminUserCreateRequest | null> = [];
    const batchSize = 25;

    postMessage({ type: 'progress', progress: 0 });
    for (let i = 0; i < rows.length; i++) {
      const validated = validateRow(rows[i], existingEmails, emailInFile);
    ...
    postMessage({
      type: 'result',
      summary: toSummary(rows.length, results),
      payloads
    });
  } catch {
    postMessage({ type: 'error', error: 'Impossible de valider le fichier CSV.' });
  }
});
...
): { result: RowValidationResult; payload: AdminUserCreateRequest | null } {
  const issues: RowValidationIssue[] = [];
  const emailRaw = (row.mapped.email ?? '').trim().toLowerCase();

  validateIdentity(row, issues);
  validateEmail(row, emailRaw, issues);
  validateRole(row, issues);
  validateDuplicates(emailRaw, emailInFile, row.rowNumber, issues);
  validateExistingEmail(emailRaw, existingEmails, issues);

  const roleRaw = (row.mapped.role ?? '').trim();
  const role = roleRaw ? normalizeRole(roleRaw) : null;
  if (roleRaw && !role) {
    issues.push({ field: 'role', severity: 'error', message: 'Rôle invalide (SWIMMER, COACH ou VISITOR)' });
  } else if (role === 'ADMIN') {
    issues.push({ field: 'role', severity: 'error', message: 'Le rôle Administrateur est interdit à l\'import CSV' });
  } else if (role) {
    row.mapped.role = role;
    validateRoleSpecific(row, role, issues);
  }

  const payload = issues.some((i) => i.severity === 'error')
    ? null
    : toPayload(row, role as ImportableRole);
...
function validateIdentity(row: MappedUserRow, issues: RowValidationIssue[]): void {
  if (!row.resolvedFirstName.trim()) {
    issues.push({ field: 'identity', severity: 'error', message: 'Prénom manquant (nom complet ou colonne prénom)' });
  }
  if (!row.resolvedLastName.trim()) {
    issues.push({ field: 'identity', severity: 'error', message: 'Nom manquant (nom complet ou colonne nom)' });
  }
}
...
function validateRole(row: MappedUserRow, issues: RowValidationIssue[]): void {
  if (!(row.mapped.role ?? '').trim()) {
...
function validateDuplicates(
  ...
  if (first !== undefined) {
    issues.push({ field: 'email', severity: 'error', message: `Email en double avec la ligne ${first}` });
  } else {
...
function validateExistingEmail(
  ...
  if (email && existingEmails.has(email)) {
    issues.push({ field: 'email', severity: 'error', message: 'Cet email existe déjà dans la base' });
  }
}

function validateRoleSpecific(row: MappedUserRow, role: ImportableRole, issues: RowValidationIssue[]): void {
  ...
  for (const field of fields) {
    ...
    if (!isFieldAllowedForRole(field, role) && value) {
      issues.push({ field, severity: 'warning', message: `Champ ignoré pour le rôle ${role}` });
      continue;
    }
    if (isFieldRequiredForRole(field, role) && !value) {
      issues.push({ field, severity: 'error', message: `${field} obligatoire pour ${roleLabel(role)}` });
    }
  }

  const gender = normalizeGender(row.mapped.gender ?? '');
  ...
  const birthDate = normalizeDate(row.mapped.birthDate ?? '');
  ...
  if (role === 'SWIMMER') {
    const discipline = normalizeDiscipline(row.mapped.discipline ?? '');
    const niveau = normalizeNiveau(row.mapped.niveau ?? '');
...
function toPayload(row: MappedUserRow, role: ImportableRole): AdminUserCreateRequest {
...
function toSummary(totalRows: number, results: RowValidationResult[]): ImportValidationSummary {
...
function roleLabel(role: ImportableRole): string {
...
function normalizeRole(raw: string): ImportableRole | 'ADMIN' | null {
  const n = raw
    .trim()
    .toUpperCase()
    .normalize('NFD')
...
function normalizeNiveau(raw: string): string | null {
  const n = raw.trim().toUpperCase();
