    ...
    public List<RankingEntryDTO> getNationalRanking(Integer distance, StrokeType stroke, Gender gender, Niveau niveau, Integer limit) {
        ...
        List<Performance> performances =
                performanceRepository.findTop50ByDistanceAndStrokeAndSwimmerGenderOrderByTimeAsc(
                        distance, stroke, gender,niveau
                );

        List<Performance> nationalRecords = performanceRepository .findNationalRecord(distance, stroke, gender);
        AtomicInteger rankCounter = new AtomicInteger(1); return performances.stream() .map(p -> RankingEntryDTO.builder() .rank(rankCounter.getAndIncrement()) .swimmerId(p.getSwimmer().getId()) .swimmerFirstName(p.getSwimmer().getFirstName()) .swimmerLastName(p.getSwimmer().getLastName()) .category(p.getSwimmer().getNiveau()) .time(p.getTime()) .date(p.getDate()) .distance(p.getDistance()) .stroke(p.getStroke()) .isNationalRecord(nationalRecords.stream().anyMatch(r -> r.getId().equals(p.getId()))).clubName(p.getSwimmer().getClub() != null ?
                p.getSwimmer().getClub().getName() : null) .build()) .toList(); }
    @Override
